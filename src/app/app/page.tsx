...
import { onboardingSteps } from "@/content/workspace";
    ...
    <main className="min-h-screen bg-[var(--bg0)] text-[var(--text)]">
      ...
      <section className="content-rail py-12">
        <div className="grid gap-8 lg:grid-cols-[0.75fr_0.25fr]">
          <div>
            <p className="mono-label text-[var(--jade)]">Workspace</p>
            <h1 className="mt-4 text-4xl font-semibold leading-tight md:text-6xl">
              Create your first living project story.
            </h1>
            <p className="mt-5 max-w-3xl leading-7 text-[var(--text-dim)]">
              Welcome, {user.name}. This workspace is ready for project creation, demo review and source connection. GitHub repository access remains a separate authorization step after login.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              >
                View VisePanda demo <ExternalLink size={16} />
              </a>
            </div>
          </div>

          <aside className="border border-[var(--line)] bg-[var(--bg1)] p-5" style={{ borderRadius: "12px" }}>
            <p className="mono-label text-[var(--text-faint)]">Signed in as</p>
            ...
            <p className="mt-4 inline-flex items-center gap-2 text-xs text-[var(--text-dim)]">
              <LockKeyhole size={14} /> {user.provider === "preview" ? "Preview auth session" : "Supabase session"}
            </p>
          </aside>
        </div>
      </section>
      ...
      <section className="content-rail pb-16">
        <div className="grid gap-8 lg:grid-cols-[0.38fr_0.62fr]">
          <div className="border-t border-[var(--line-hi)] pt-5">
            <p className="mono-label text-[var(--jade)]">Empty project state</p>
            <h2 className="mt-4 text-2xl font-semibold">No projects yet.</h2>
            <p className="mt-3 text-sm leading-6 text-[var(--text-dim)]">
              Start with a project profile, then connect GitHub metadata and review imported activity before publishing any view.
            </p>
          </div>
